...
for (i in seq_along(ema1)) {
  for (j in seq_along(ema2)) {
    if (ema1[i] >= ema2[j]) {
      ...
      matriz_resultados[i, j] <- calcular_saldo_final_ema(datos.filt, ema1[i], ema2[j])
    }
...
plot_ly(
  ...
  hovertemplate = paste(  # Formato personalizado para el tooltip
    "Fila: %{y}<br>",
    "Columna: %{x}<br>",
    "Valor: %{z:.2e}<extra></extra>"  # %{z:,} añade separadores de miles
  ),
  type = "heatmap"
) %>%
