...
# Aplicar la simulación azarosa a cada ticker
resultados_por_ticker_azar <- lapply(tickers_split, simular_estrategia_azar)

# Unir todos los resultados en un único dataframe
resultados_finales_azar <- bind_rows(resultados_por_ticker_azar)

```

```{r}
saldo_final_por_ticker_azar <- resultados_finales_azar %>%
  group_by(Ticker) %>%
  filter(!is.na(saldo)) %>%
  slice_tail(n = 1) %>%
  select(Ticker, Date, saldo)

print(saldo_final_por_ticker_azar)
```
